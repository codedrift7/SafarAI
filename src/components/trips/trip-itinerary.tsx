...
export function TripItinerary({ initialTrip, initialMessages, initialAdvisories, readOnly = false }: { initialTrip: Trip; initialMessages?: ChatMessage[]; initialAdvisories?: Advisory[]; readOnly?: boolean }) {
  ...
  return <section className="bg-sandstone-mist"><div className="border-b border-karakoram-ink/10 bg-white px-4 py-4 sm:px-6 lg:px-8"><div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><Link href="/trips" className="grid size-9 shrink-0 place-items-center rounded-full text-karakoram-ink/65 hover:bg-sandstone-mist" aria-label="Back to trips"><ArrowLeft size={18} /></Link><div className="min-w-0"><p className="truncate text-xs font-bold uppercase tracking-[.12em] text-attabad-turquoise">{readOnly ? "Shared itinerary · read only" : "Your trip"}</p><h1 className="display-type truncate text-2xl leading-tight sm:text-3xl">{trip.title}</h1></div></div><div className="flex items-center gap-2">{!readOnly && <Button onClick={share} size="sm" variant="secondary"><Share2 size={15} /> Share</Button>}<Button onClick={download} size="sm" variant="secondary"><Download size={15} /> Export</Button></div></div></div>{shareUrl && <div className="border-b border-attabad-turquoise/20 bg-attabad-turquoise/8 px-4 py-2 text-center text-xs text-[#155367]">Share link is ready{!readOnly && ": copied to your clipboard"}. <Link href={shareUrl} className="font-bold underline underline-offset-2">Open read-only view</Link></div>}<div className="mx-auto grid max-w-[1500px] gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8"><div className="min-w-0"><div className="rounded-[1.4rem] bg-karakoram-ink p-5 text-sandstone-mist sm:p-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><Badge variant="marigold">{trip.travelerType.toLowerCase()}</Badge><Badge variant="marigold">{trip.pace} pace</Badge>{trip.budgetTier && <Badge variant="marigold">{trip.budgetTier.replace("_", " ")}</Badge>}</div><p className="mt-4 max-w-xl text-sm leading-6 text-sandstone-mist/72">{trip.vibe ?? "A thoughtful route through real places."}</p></div><div className="rounded-xl border border-white/12 bg-white/6 px-3 py-2 text-xs text-sandstone-mist/72"><span className="mono-type text-truck-art-marigold">{new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(new Date(trip.startDate))}</span> → <span className="mono-type text-truck-art-marigold">{new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(new Date(trip.endDate))}</span></div></div></div><div className="mt-5 rounded-[1.4rem] border border-karakoram-ink/12 bg-white p-3 sm:p-4"><div className="flex items-center justify-between gap-3 px-2 pb-3"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-attabad-turquoise">Today’s leg</p><h2 className="display-type mt-1 text-2xl">Day {activeDay?.dayNumber ?? "—"}</h2></div><span className="inline-flex items-center gap-1.5 text-xs font-medium text-karakoram-ink/56"><MapPin size={14} className="text-attabad-turquoise" /> {activeDay?.region?.name ?? "Pakistan"}</span></div><div className="mb-4 flex gap-2 overflow-x-auto border-y border-karakoram-ink/8 py-3" role="tablist" aria-label="Itinerary days">{trip.days.map((day) => <button role="tab" aria-selected={day.dayNumber === activeDay?.dayNumber} key={day.id} onClick={() => setActiveDayNumber(day.dayNumber)} className={`min-w-[92px] rounded-xl px-3 py-2 text-left text-xs transition ${day.dayNumber === activeDay?.dayNumber ? "bg-attabad-turquoise text-white" : "text-karakoram-ink/62 hover:bg-sandstone-mist"}`}><span className="block font-bold">Day {day.dayNumber}</span><span className="mt-1 block opacity-75">{formatDay(day.date)}</span></button>)}</div><TripMap day={activeDay} /></div><div className="mt-5 rounded-[1.4rem] border border-karakoram-ink/12 bg-sandstone-mist/70 p-4 sm:p-6"><div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-attabad-turquoise">The Karakoram Line</p><h2 className="display-type mt-1 text-3xl">A day in sequence.</h2></div><p className="max-w-xs text-sm leading-5 text-karakoram-ink/60">A route line for the things that happen in order, not just the places you pin.</p></div>{activeDay ? <KarakoramLine activities={activeDay.activities} advisories={advisories} onMove={readOnly ? undefined : handleMove} onDelete={readOnly ? undefined : handleDelete} /> : <p className="py-10 text-center text-karakoram-ink/60">No days are ready yet.</p>}{!readOnly && <form onSubmit={handleAdd} className="mt-2 flex flex-col gap-2 rounded-xl border border-dashed border-karakoram-ink/22 bg-white p-3 sm:flex-row"><label className="sr-only" htmlFor="custom-stop">Add a custom stop</label><input id="custom-stop" value={newStop} onChange={(event) => setNewStop(event.target.value)} className="min-w-0 flex-1 bg-transparent px-2 py-2 text-sm outline-none" placeholder="Add a custom stop to this day…" /><Button type="submit" size="sm" disabled={adding || !newStop.trim()}>{adding ? <LoaderCircle className="animate-spin" size={15} /> : <Plus size={15} />} Add stop</Button></form>}</div></div>{!readOnly && <aside className="rounded-[1.4rem] border border-karakoram-ink/12 bg-white lg:sticky lg:top-[88px] lg:h-[calc(100vh-110px)] lg:max-h-[800px]"><div className="border-b border-karakoram-ink/10 p-5"><div className="flex items-center gap-2"><span className="grid size-8 place-items-center rounded-xl bg-attabad-turquoise/12 text-attabad-turquoise"><Sparkles size={17} /></span><div><h2 className="font-bold">Ask Safar</h2><p className="text-xs text-karakoram-ink/58">Try “swap a fort for something outdoors”.</p></div></div></div><div className="max-h-72 space-y-4 overflow-y-auto p-5 lg:max-h-none lg:h-[calc(100%-141px)]">{orderedMessages.length === 0 && <div className="rounded-xl bg-sandstone-mist p-4 text-sm leading-6 text-karakoram-ink/70">Want a slower morning, more food, or a different kind of stop? Tell me what you’re after.</div>}{orderedMessages.map((message) => <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><p className={`max-w-[88%] rounded-2xl px-3 py-2.5 text-sm leading-5 ${message.role === "user" ? "bg-karakoram-ink text-sandstone-mist" : "bg-sandstone-mist text-karakoram-ink"}`}>{message.content}</p></div>)}{sending && <div className="flex items-center gap-2 text-xs text-karakoram-ink/60"><LoaderCircle className="animate-spin text-attabad-turquoise" size={15} /> Safar is adjusting your route…</div>}</div><form onSubmit={handleChat} className="border-t border-karakoram-ink/10 p-3"><label className="sr-only" htmlFor="chat-edit">Ask Safar to edit your itinerary</label><div className="flex items-end gap-2 rounded-xl border border-karakoram-ink/14 bg-sandstone-mist/50 p-2"><textarea id="chat-edit" value={chatValue} onChange={(event) => setChatValue(event.target.value)} className="min-h-10 max-h-24 flex-1 resize-none bg-transparent px-1 py-2 text-sm outline-none" placeholder="Change this route…" /><button type="submit" disabled={!chatValue.trim() || sending} className="grid size-9 shrink-0 place-items-center rounded-lg bg-attabad-turquoise text-white disabled:opacity-45" aria-label="Send message"><Send size={16} /></button></div></form></aside>}</div></section>;
}
