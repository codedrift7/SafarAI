...
export function SiteHeader() {
  ...
  // When not logged in, "Plan a trip" sends to login with redirect back
  const planHref = auth.user ? "/trips/new" : "/login?redirect=/trips/new";
    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-karakoram-ink/95 text-sandstone-mist backdrop-blur">
      <div className="mx-auto flex h-[68px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-7 md:flex" aria-label="Primary navigation">
          ...
          <Button asChild size="sm"><Link href={planHref}><Plus size={16} strokeWidth={2} /> Plan a trip</Link></Button>
          <UserMenu />
        </nav>
      ...
      {open && <nav className="border-t border-white/10 bg-karakoram-ink px-4 py-4 md:hidden" aria-label="Mobile navigation">
        <div className="mx-auto flex max-w-7xl flex-col gap-1">
          ...
          <Button asChild className="mt-2"><Link onClick={() => setOpen(false)} href={planHref}><Plus size={16} /> Plan a trip</Link></Button>
