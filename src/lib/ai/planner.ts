...
function systemPrompt(input: PlannerInput): string {
  const parts = [
    ...
    "If suggesting a non-catalogue stop, set poiId to null and provide customTitle and note. Set the claims object to declare any specific checkable claims you assert: placeName=true if asserting a specific venue or business name, address=true ONLY if asserting an explicit physical street address (e.g., '12 Mall Road'), hours=true if asserting opening/closing hours, and price=true if asserting an entry fee or cost. For general markets, bazaars, and stops without entry fees, leave price and address false. If suggesting a generic activity (e.g. 'take a short break at a local tea house', 'explore the bazaar'), omit or leave claims false.",
